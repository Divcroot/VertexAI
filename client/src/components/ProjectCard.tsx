...
const ProjectCard = ({ project }: ProjectCardProps) => {
    ...
    const { handleToggleStar: toggleStar, handleDeleteProject } = useProjects();

    const handleToggleStar = async (e: MouseEvent<HTMLButtonElement>): Promise<void> => {
        e.stopPropagation();
        setLoadingStar(true);
    ...
    return (
        ...
        >
            ...
            <motion.button
                ...
                className={`absolute right-4 top-4 rounded-md p-1 transition-opacity hover:text-amber-400 ${project.starred ? "opacity-100 text-amber-400" : "opacity-0 text-zinc-300 group-hover:opacity-100 dark:text-zinc-600"}`}
            >

                <Star size={15} className={project.starred ? "fill-amber-400 text-amber-400" : ""} />
